...
max = 8000 # Max storage (Free)
premium = 8000 # Max storage (Premium)
...
async def help(ctx):
    if ctx.guild.id == 871118468043399169:
        invitemenu = discord.Embed(title="Invite Me",description="Click [here](https://discord.com/api/oauth2/authorize?client_id=987308292172824576&permissions=8&scope=bot) to invite me to your own server",color=color)
        ...
        helpmenu.description = "For support, join the discord [here](https://discord.gg/3cTwMaquBZ)"
...
async def status():
	await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=f"{len(bot.guilds)} discord.gg/3invite"))
    ...
    if name == None:
        ...
        if data["channel"] == 0:
            ...
            if ctx.channel.id != data["channel"]:
                channel = data["channel"]
                await ctx.send(f"Gen channel is <#{channel}>",delete_after=10)
            else:
                name = name.lower()
                if data["services"] == {}:
                    ...
                    if name not in data["services"]:
                        ...
                        try:
                            ...
                            accountmenu.set_footer(text="ID:PASS")
                            msg = await ctx.author.send(embed=accountmenu,delete_after=99991119839218475)
                        except:
                            ...
                            with open(f"Guilds/{ctx.guild.id}.json","w") as file:
                                json.dump(data,file,indent=4)
                            success = discord.Embed(title="success!",description=f"check i dms you [here]({msg.jump_url}) ",color=color)
            ...
            if "paste.ee/" in link:
                ...
                if a.status_code != 200:
                    ...
                    with open(f"Guilds/{ctx.guild.id}.json","w") as file:
                        json.dump(data,file,indent=4)
                    if added == 0:
                        success = discord.Embed(title="No Lines Added",description=f"Max storage reached!\n\nTo get more storage, join the discord [here](https://discord.gg/3cTwMaquBZ)",color=color)
                    else:
...
async def stock(ctx):
    ...
    if stock.description != "":
        if other["premium"] == 0:
            ...
            if percent > 75:
                stock.description += "\nTo get more storage, join the discord [here](https://discord.gg/3cTwMaquBZ)"
        if other["premium"] == 1:
